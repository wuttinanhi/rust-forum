...
static NAME_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^[a-zA-Z0-9 ]{2,32}$").unwrap());
...
pub struct UserRegisterFormData {
    #[validate(regex(
        path = *NAME_REGEX,
        message = "Name must contain only letters, numbers and spaces. length between 2 and 32."
    ))]
    pub name: String,
